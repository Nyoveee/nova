...
bool Editor::isChildEntitySelected(entt::registry& registry, entt::entity entity,entt::entity root)
{
...
void Editor::startSimulation() {
	...
	Serialiser::serialiseScene(engine.ecs.registry, engine.ecs.sceneManager.layers, engine.ecs.sceneManager.currentSceneProperties, filePath->string.c_str());
...
void Editor::displayEntityHierarchy(
	...
	if (!toRecurse || entityData->children.empty()) {
		...
		if (isChildEntitySelected(registry, entity, entity)) {
