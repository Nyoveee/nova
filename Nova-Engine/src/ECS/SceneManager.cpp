...
void SceneManager::loadScene(ResourceID id) {
	...
	Serialiser::deserialiseScene(ecs.registry, layers, currentSceneProperties, scene->getFilePath().string.c_str());
...
void SceneManager::editorChangeScene(ResourceID id, AssetFilePath const& sceneAssetFilepath) {
	...
	Serialiser::deserialiseScene(ecs.registry, layers, currentSceneProperties, sceneAssetFilepath.string.c_str());
	currentScene = id;
