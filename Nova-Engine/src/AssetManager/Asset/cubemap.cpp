...
void CubeMap::load(AssetManager& assetManager) {
	if (isLoaded()) {
	...
	assetManager.threadPool.detach_task([&]() {
		...
		// copies the pointer `data` and other local variables..
		// back to main thread, we can submit data to GPU.
		assetManager.submitCallback([data, err, status, width, height, this]() {
