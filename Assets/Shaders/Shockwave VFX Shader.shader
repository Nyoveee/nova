...
Frag{
    ...
    float NdotV = max(dot(fsIn.normal, viewDir), 0.0);
    float fresnelFactor = pow(1.0 - NdotV, 1);

    vec3 color = mix(colorOne, colorTwo, noise(fsIn.boundingBoxUVW.x * scale + timeElapsed * speedMultiplier));
    float alpha = noise(fsIn.boundingBoxUVW.xz * vec2(scale) + vec2(timeElapsed * speedMultiplier));
	return vec4(color * emissivemultiplier, fresnelFactor);
}