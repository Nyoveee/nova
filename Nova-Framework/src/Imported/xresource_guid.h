...
namespace xresource
{
    ...
    struct instance_guid_large
    {
        ...
        bool operator == (const instance_guid_large& B) const noexcept
        {
            assert(isValid() || empty());
            assert(B.isValid() || B.empty());
            return guid.m_Low == B.guid.m_Low && guid.m_High == B.guid.m_High;
        }
        ...
        bool operator < (const instance_guid_large& B) const noexcept
        {
            assert(isValid() || empty());
            assert(B.isValid() || B.empty());
            return guid.m_Low < B.guid.m_Low || (guid.m_Low == B.guid.m_Low && guid.m_High < B.guid.m_High);
        }
        ...
        [[nodiscard]] bool empty() const noexcept
        {
            return guid.m_Low == 0 && guid.m_High==0;
        }

        inline void clear() noexcept
        {
			guid.m_Low = guid.m_High = 0;
        }

        constexpr
        [[nodiscard]] bool isValid() const noexcept
        {
            return (guid.m_Low | guid.m_High) && !!(guid.m_Low & 1);
        }
        ...
        [[nodiscard]] instance_guid_large generate_guid(const char* str, uint64_t h1 = 0x548c9decbce65297ULL, uint64_t h2 = 0x548c9decbce65297ULL, int len = 0) noexcept
        {
            ...
            return (*str == '\0') ?
                ...
                [](uint64_t h1, uint64_t h2, int len) consteval noexcept -> instance_guid_large
                {
					...
					return instance_guid_large{ .guid = {h1, h2} };
                }(h1, h2, len) :
...
namespace std
{
    ...
    struct hash<xresource::instance_guid_large>
    {
        ...
        {
            std::size_t h1 = std::hash<uint64_t>{}(k.guid.m_Low);
            std::size_t h2 = std::hash<uint64_t>{}(k.guid.m_High);
